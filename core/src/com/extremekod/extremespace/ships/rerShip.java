...
public class rerShip extends Ship {

    private TextureRegion texture;

    public rerShip(int x, int y, boolean target) {
        super(x, y, target);

        this.texture = new TextureRegion(Es_Resources.rerShipTexture);
    }

    public void fireLightLaser (Stage stage){
        stage.addActor(new LightLaser(this.getX(), this.getY() + CENTRAL_FIRE_POINT, target, this));
    }

    public void fireRocket (Stage stage){
        stage.addActor(new Rocket(this.getX(), this.getY() + CENTRAL_FIRE_POINT, target, this));
    }
